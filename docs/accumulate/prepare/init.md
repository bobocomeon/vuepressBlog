...
### keep-alive lru 浏览器缓存淘汰策略
- 嵌套route-view，可以对组件页面进行缓存，首次渲染的时候设置缓存，有include和exclude两个属性，使用的是lru 缓存淘汰算法，数据结果是一个链表，如果数据最近被访问过，那么将来被访问的几率会更高，就移到链表的头结点，当链表满的时候，将链表尾部的数据丢弃。
